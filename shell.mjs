...
console.info('Start shell access ...');
...
function requireReact_production() {
  ...
  react_production.version = "19.0.0-rc-8b08e99e-20240713";
...
class Idle {
  constructor() {
    ...
    __publicField(this, "registerNetworRequest", () => {
      const id = {};
      this.request.add(id);
    ...
    __publicField(this, "whenIdle", (callback) => {
      const id = {};
      ...
      setTimeout(unregisterContentLoaded, 3e3);
    ...
    this.request = /* @__PURE__ */ new Set();
    this.callback = /* @__PURE__ */ new Map();
  }
}
const idle = new Idle();
const registerNetworRequest = idle.registerNetworRequest;
idle.whenIdle;
const timeout = (time) => {
...
const libF = async () => {
  for (let i = 0; i < 10; i++) {
    console.info(`iteracja ${i}`);
    await timeout(1e3);
  }
};
const main = async () => {
  console.info("Schelowy scyzoryk szwajcarski");
  await libF();
};
main().then(() => {
  console.info("shell end");
  process.exit(0);
}).catch((error) => {
